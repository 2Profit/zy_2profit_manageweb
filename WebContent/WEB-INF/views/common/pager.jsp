	...
	<div style="text-align: right; border: 0;padding: 4px 12px;" class="pageDiv">
		...
		<pg:pager items="${page.totalCount}"
			...
			maxPageItems="${page.pageSize}" maxIndexPages="10" isOffset="true">
					...
					<pg:param name="cc" />
			<pg:first>
				<a name="first" class="pageLink">首页</a>
			</pg:first>
			<pg:prev>
				<a name="prev" class="pageLink">上一页</a>
			</pg:prev>
			<pg:pages>
				<c:choose>
					...
					<c:otherwise>
						<a name="doNumberPage" class="pageLink">${pageNumber}</a>
					</c:otherwise>
				</c:choose>
			</pg:pages>
			<pg:next>
			    <a name="next" class="pageLink">下一页</a>
			</pg:next>
			<pg:last>
				<a name="last" class="pageLink">尾页</a>
			</pg:last>
		</pg:pager>
	</div>
</body>
